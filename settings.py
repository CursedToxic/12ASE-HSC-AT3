...
def option_row(parent, options, current_var, start_row):
    ...
    for i, (label, value) in enumerate(options):
        ...
        ctk.CTkRadioButton(
            card, text=label, variable=current_var, value=value,
            fg_color=PURPLE, hover_color=PURPLE_HOVER,
            font=ctk.CTkFont(size=13)
        ).grid(row=i, column=0, padx=20, pady=(top_pad, bottom_pad), sticky="w")


class SettingsFrame(ctk.CTkFrame):
    """
    Settings page. Calls on_apply(new_settings) when the user clicks Apply.
    settings dict: {"appearance": str, "font_scale": float}
    """
