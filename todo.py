...
class TodoFrame(ctk.CTkFrame):
    def __init__(self, master):
        ...
        ctk.CTkLabel(self, text="To-Do List", font=ctk.CTkFont(size=20, weight="bold")).grid(row=0, column=0, padx=20, pady=(20, 4), sticky="w")

        input = ctk.CTkFrame(self, fg_color="transparent")
        input.grid(row=1, column=0, padx=20, pady=(0, 8), sticky="ew")
        input.grid_columnconfigure(0, weight=1)
        self.task_entry = ctk.CTkEntry(input, placeholder_text="Add a task…", height=34)
        self.task_entry.grid(row=0, column=0, sticky="ew", padx=(0, 8))
        self.task_entry.bind("<Return>", lambda _: self.add_task())
        add_btn = primary_btn(input, "+ Add", self.add_task, width=72)
    ...
    def render_tasks(self):
        ...
        for group_label, group in [("Pending", pending), ("Completed", done)]:
            if not group:
                continue
            ctk.CTkLabel(self.list_area, text=group_label.upper(), font=ctk.CTkFont(size=10), text_color="gray50").grid(row=row_idx, column=0, sticky="w", padx=4, pady=(8, 2))
            row_idx += 1
            for task in group:
                ...
                toggle_btn = ctk.CTkButton(row, text="✓" if done_flag else "○", width=32, height=32, corner_radius=16, fg_color=PURPLE if done_flag else "transparent",
                ...
                delete_button = danger_btn(row, "🗑", lambda i=real_idx: self.delete_task(i))
